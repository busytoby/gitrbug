...
class QueuedTask extends AppModel {
	...
	public function requestJob($capabilities) {
		...
		$findConf = array(
			...
			'fields' => array(
				...
				'completed',
				'failed',
				'strftime("%s", datetime("now")) - strftime("%s", notbefore) AS age'
			),
			'order' => array(
				'age DESC',
				'id ASC'
	...
	public function getStats() {
		$findConf = array(
			'fields' => array(
				'jobtype, count(id) as num, AVG(strftime("%s", completed)-strftime("%s", created)) AS alltime, AVG(strftime("%s", completed)-strftime("%s", fetched)) AS runtime, AVG(strftime("%s", fetched-strftime("%s", IFNULL(notbefore,created)))) AS fetchdelay'
			),
			'conditions' => array(
				'NOT' => array(
					'completed' => 'NULL'
